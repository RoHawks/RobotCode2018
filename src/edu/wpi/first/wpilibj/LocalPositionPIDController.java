...
public class LocalPositionPIDController implements LiveWindowSendable, Controller {
	...
	private double m_izone = Configurables.ERROR_MIN;
	private WPI_TalonSRX m_pidInput;
	private WPI_TalonSRX m_pidOutput;
	...
	private class PIDTask extends TimerTask {
		...
		private LocalPositionPIDController m_controller;

		public PIDTask(LocalPositionPIDController controller) {
			if (controller == null) {
	 ...
	 *            The default is 50ms.
	 */
	public LocalPositionPIDController(double Kp, double Ki, double Kd, double Kf, WPI_TalonSRX source,
			WPI_TalonSRX output, double period) {
	 ...
	 *            the feed forward term
	 * @param talon
	 *            pid_input and pid_output
	 * @param period
	 *            the loop time for doing calculations. This particularly
	 *            effects calculations of the integral and differential terms.
	 *            The default is 50ms.
	 */
	public LocalPositionPIDController(double Kp, double Ki, double Kd, double Kf, WPI_TalonSRX talon, double period) {
		this(Kp, Ki, Kd, Kf, talon, talon, period);
	}
	 ...
	 *            The default is 50ms.
	 */
	public LocalPositionPIDController(double Kp, double Ki, double Kd, WPI_TalonSRX source, WPI_TalonSRX output,
			double period) {
	 ...
	 *            The default is 50ms.
	 */
	public LocalPositionPIDController(double Kp, double Ki, double Kd, WPI_TalonSRX source, double period) {
		this(Kp, Ki, Kd, 0.0, source, source, period);
	}
	 ...
	 *            The PIDOutput object that is set to the output percentage
	 */
	public LocalPositionPIDController(double Kp, double Ki, double Kd, WPI_TalonSRX source) {
		this(Kp, Ki, Kd, source, kDefaultPeriod);
	}

	public synchronized void resetIGain() {
		m_ITerm = 0;
	}
	...
	private void calculate() {
		...
		if (enabled) {
			...
			synchronized (this) {
				input = ResourceFunctions.tickToAngle(pidInput.getSelectedSensorPosition(0));
				if (input == Double.NaN) {
			...
			synchronized (this) {
				...
				if (m_I != 0) {
					if (m_izone < Configurables.ERROR_MIN - 1) {
				...
				if (Math.abs(m_error) < m_deadband) {
					m_result = 0;
				}
	...
	public synchronized double getError() {
		// return m_error;
		double input = ResourceFunctions.tickToAngle(m_pidInput.getSelectedSensorPosition(0));
		if (input == Double.NaN)
			return 0;
